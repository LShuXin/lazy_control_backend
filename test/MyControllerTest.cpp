...
void MyControllerTest::onRun() {
  ...
  /* Add LoginController endpoints to the router of the test server */
  ...
  runner.run([this, &runner] {
    ...
    /* Call server API */
    /* Call hello endpoint of LoginController */
    ...
    auto message = response->readBodyToDto<oatpp::Object<LoginResponseDto>>(objectMapper.get());

    /* Assert that received message is as expected */
    OATPP_ASSERT(message);
    OATPP_ASSERT(message->statusCode == 200);
    OATPP_ASSERT(message->data == "Hello World Async!");
